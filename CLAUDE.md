...
## Session Resume Guide (last updated 2026-06-08)

This section exists so Claude Code can pick up immediately in a new session without asking the user to re-explain context.

### Who is the user

Christiaan Verhoef (kamitor on GitHub). Dutch. Based in Baarn, NL. Fedora Linux on `100.95.236.57` (Tailscale). Seeking 2-3 concurrent part-time/fractional roles. See `memory/MEMORY.md` for full profile.

### Repository status

- **Repo**: `TheSocialChicken/CareerBuilder` — new standalone repo (was fork of `santifer/career-ops`, detached by migration)
- **Old repo**: `TheSocialChicken/career-ops` — still exists, can be deleted
- **Local path**: `/home/chris/Nextcloud/Github/career-ops`
- **Remote**: `git@github.com:TheSocialChicken/CareerBuilder.git`
- **Pages**: `https://thesocialchicken.github.io/CareerBuilder/` — enabled, served from `docs/`
- **Branch protection**: main is default branch

### GitHub Secrets (TheSocialChicken/CareerBuilder)

| Secret | Value / Notes |
|--------|---------------|
| `GH_PAT` | Full-repo PAT for kamitor — see KeePass |
| `OPENROUTER_API_KEY` | unlabeled-2 key — see KeePass |
| `N8N_BASE_URL` | `https://n8n.valuechainhackers.xyz` |
| `TAILSCALE_LOCAL_IP` | `100.95.236.57` |
| `N8N_API_KEY` | **NOT YET SET** on new repo — user needs to provide |

### N8N setup status

All 5 pipelines created and live in N8N. Workflow JSONs in `n8n/workflows/`.

**DONE:**
- All 5 pipelines created and active in N8N
- Workflow JSONs committed to repo
- OpenRouter free models configured (`n8n/openrouter-models.json`)
- GitHub Secrets set (except `N8N_API_KEY` on new repo)

**PENDING — user must do:**
1. Set N8N Variables in N8N UI (Settings → Variables):
   ```
   LOCAL_API_URL       = http://100.95.236.57:3000
   OPENROUTER_API_KEY  = (from KeePass → OpenRouter → unlabeled-2-active)
   GITHUB_TOKEN        = (from KeePass → GitHub → GH-PAT-kamitor)
   GITHUB_REPO         = TheSocialChicken/CareerBuilder
   SCAN_MODEL          = meta-llama/llama-3.2-3b-instruct:free
   EVAL_MODEL          = nvidia/nemotron-3-ultra-550b-a55b:free
   CV_MODEL            = moonshotai/kimi-k2.6:free
   REPORT_MODEL        = nvidia/nemotron-3-ultra-550b-a55b:free
   ```
2. Start local API bridge on Fedora machine:
   ```bash
   npm run api
   ```
3. Trigger pipeline 03 (Dashboard) once manually in N8N to populate GitHub Pages
4. Add `N8N_API_KEY` secret to CareerBuilder repo (need value from user or N8N UI)

### Local API bridge (`api.mjs`)

Runs on `100.95.236.57:3000`. N8N → Tailscale → this bridge → local files.

```bash
npm run api         # foreground
npm run api &       # background (nohup recommended for persistence)
```

Verify: `curl http://100.95.236.57:3000/health`

### Embeddings (RAG)

Ollama on `100.95.236.57:11434`. `nomic-embed-text` — **ALREADY PULLED**.

### KeePass credentials

File: `~/CareerBuilder.kdbx` (on Christiaan's Fedora machine)
Password hint: in `memory/openrouter-keys.md` — do NOT put in this file (repo-committed)

Contents:
- OpenRouter → 7 keys (labeled by account)
- GitHub → GH-PAT-kamitor
- N8N → base URL

### Key decisions made

- OpenRouter free models used throughout — no paid tier needed
- Each pipeline gets its own N8N workflow (separate IDs) so models can be swapped independently
- Local machine used for: PDF generation (Playwright), embeddings (Ollama), file I/O (api.mjs)
- N8N hosted server used for: all LLM calls, scheduling, webhook reception
- `docs/index.html` is auto-generated by pipeline 03 — do not manually edit
